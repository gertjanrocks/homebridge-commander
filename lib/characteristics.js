...
//
//  Status Active Get is used for:
//    Temperature Sensor
//
exports.getStatusActive = function(callback) {
  var that = this;
  var cmd = that.cmd;

  // Add arguments
  if(!that.no_arg){
    cmd += " '" + that.name + "' statusactive" + " get";
  }
  // Execute command to get Status Active
  exec(cmd, function (error, stdout, stderr) {
    //Get status Active
    that.statusactive = (stdout.trim() === "true") ? true : false;
    // Error detection
    if (stderr) {
      that.log("Failed to excecute get command for",that.name);
      that.log(stderr);
    ...
    if (callback) {
      callback(stderr, that.statusactive);
    }
  });
}

//
//  Status Fault Get is used for:
//    Temerature Sensor
//
exports.getStatusFault = function(callback) {
  var that = this;
  var cmd = that.cmd;

  // Add arguments
  if(!that.no_arg){
    cmd += " '" + that.name + "' statusfault" + " get";
  }
  // Execute command to get status fault
  exec(cmd, function (error, stdout, stderr) {
    //Get status fault
    that.statusfault = parseInt(stdout);  
    // Error detection
    if (stderr) {
      that.log("Failed to excecute get command for",that.name);
      that.log(stderr);
    ...
    if (callback) {
      callback(stderr, that.statusfault);
    }
  });
}

//
//  Status Low Battery Get is used for:
//    Temerature Sensor
//
exports.getStatusLowBattery = function(callback) {
  var that = this;
  var cmd = that.cmd;

  // Add arguments
  if(!that.no_arg){
    cmd += " '" + that.name + "' statuslowbattery" + " get";
  }
  // Execute command to get status lowbattery
  exec(cmd, function (error, stdout, stderr) {
    //Get status lowbattery
    that.statuslowbattery = parseInt(stdout);  
    // Error detection
    if (stderr) {
      that.log("Failed to excecute get command for",that.name);
      that.log(stderr);
    ...
    if (callback) {
      callback(stderr, that.statuslowbattery);
    }
  });
}

//
//  Status Tampered Get is used for:
//    Temerature Sensor
//
exports.getStatusTampered = function(callback) {
  var that = this;
  var cmd = that.cmd;

  // Add arguments
  if(!that.no_arg){
    cmd += " '" + that.name + "' statustampered" + " get";
  }
  // Execute command to get status tampered
  exec(cmd, function (error, stdout, stderr) {
    //Get status tampered
    that.statustampered = parseInt(stdout);  
    // Error detection
    if (stderr) {
      that.log("Failed to excecute get command for",that.name);
      that.log(stderr);
    ...
    if (callback) {
      callback(stderr, that.statustampered);
    }
  });
}

//
//  CurrentRelativeHumidity Get is used for:
//    Humidity Sensor
//
exports.getCurrentRelativeHumidity = function(callback) {
  var that = this;
  var cmd = that.cmd;

  // Add arguments
  if(!that.no_arg){
    cmd += " '" + that.name + "' currentrelativehumidity" + " get";
  }
  // Execute command to get Humidity
  exec(cmd, function (error, stdout, stderr) {
    //Get humidity
    that.currentrelativehumidity = parseInt(stdout);  
    // Error detection
    if (stderr) {
      that.log("Failed to excecute get command for",that.name);
      that.log(stderr);
    ...
    if (callback) {
      callback(stderr, that.currentrelativehumidity);
    }
  });
}
