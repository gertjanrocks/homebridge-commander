...
module.exports.addService = function(that) {
    ...
    switch (that.type)
    {
        ...
        {
            that.service = new Service.TemperatureSensor(that.name);
            break;
        }
        case "humiditysensor":
        {
...
module.exports.addCharacteristic = function(that) {
    ...
    //CurrentTemperature
    if( (that.type == "temperaturesensor") ||
        (that.custom && that.settings.currenttemperature))
    {
        that.service.getCharacteristic(Characteristic.CurrentTemperature)
        .on('get', that.getCurrentTemperature.bind(that));
    }

    //Status Active
    if( ((that.type == "temperaturesensor") && that.settings.statusactive) ||
    ((that.type == "humiditysensor") && that.settings.statustampered) ||
        (that.custom && that.settings.statusactive))
    {
        that.service.getCharacteristic(Characteristic.StatusActive)
        .on('get', that.getStatusActive.bind(that));
    }

    //Status Fault
    if( ((that.type == "temperaturesensor") && that.settings.statusfault) ||
        ((that.type == "humiditysensor") && that.settings.statustampered) ||
        (that.custom && that.settings.statusfault))
    {
        that.service.getCharacteristic(Characteristic.StatusFault)
        .on('get', that.getStatusFault.bind(that));
    }
   
    //Status LowBattery
    if( ((that.type == "temperaturesensor") && that.settings.statuslowbattery) ||
        ((that.type == "humiditysensor") && that.settings.statustampered) ||
        (that.custom && that.settings.statuslowbattery))
    {
        that.service.getCharacteristic(Characteristic.StatusLowBattery)
        .on('get', that.getStatusLowBattery.bind(that));
    }
    
    //Status Tampered
    if( ((that.type == "temperaturesensor") && that.settings.statustampered) ||
        ((that.type == "humiditysensor") && that.settings.statustampered) ||
        (that.custom && that.settings.statustampered))
    {
        that.service.getCharacteristic(Characteristic.StatusTampered)
        .on('get', that.getStatusTampered.bind(that));
    }
   
    //CurrentRelativeHumidity
    if( (that.type == "humiditysensor") ||
        (that.custom && that.settings.currentrelativehumidity))
    {
        that.service.getCharacteristic(Characteristic.CurrentRelativeHumidity)
        .on('get', that.getCurrentRelativeHumidity.bind(that));
    }
  }
  ...
  module.exports.updateStatus = function(that) {
    ...
    if( ((that.type == "temperaturesensor")) ||
        (that.custom && that.settings.currenttemperature))
    {
        that.service.getCharacteristic(Characteristic.CurrentTemperature).getValue();
    }
    ...
    if( ((that.type == "temperaturesensor") && that.settings.statusactive) ||
        ...
        (that.custom && that.settings.statusactive))
    {
        that.service.getCharacteristic(Characteristic.StatusActive).getValue();
    }
    ...
    if( ((that.type == "temperaturesensor") && that.settings.statusfault) ||
        ...
        (that.custom && that.settings.statusfault))
    {
        that.service.getCharacteristic(Characteristic.StatusFault).getValue();
    }
    ...
    if( ((that.type == "temperaturesensor") && that.settings.statuslowbattery) ||
        ...
        (that.custom && that.settings.statuslowbattery))
    {
        that.service.getCharacteristic(Characteristic.StatusLowBattery).getValue();
    }
    ...
    if( ((that.type == "temperaturesensor") && that.settings.statustampered) ||
        ...
        (that.custom && that.settings.statustampered))
    {
        that.service.getCharacteristic(Characteristic.StatusTampered).getValue();
    }
    ...
    if( ((that.type == "humiditysensor")) ||
        (that.custom && that.settings.currentrelativehumidity))
    {
        that.service.getCharacteristic(Characteristic.CurrentRelativeHumidity).getValue();
    }
  }